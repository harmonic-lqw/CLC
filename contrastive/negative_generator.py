    ...
    def forward(self, feats, num_patches):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if self.use_conv:
                ...
                feat = conv(feat)
            # (N, C, H, W) -> (N, H, W, C) -> (N, C)
            feat = feat.permute(0, 2, 3, 1).mean(dim=(1, 2)) # 论文图中的MEAN?
            # (N, C) -> (N, 256, C)
            feat = feat.unsqueeze(dim=1).repeat(1, num_patches, 1)
            # (N, 256, C) + (N, 256, 64) -> (N, 256, C+64) -> (N*256, C+64)
            inp = torch.cat([feat, noise], dim=2).flatten(0, 1)
            mlp = getattr(self, 'mlp_%d' % feat_id)
            # (N*256, C+64) -> (N*256, 256)
            neg_sample = mlp(inp)
...
class Negative_Placeholder(nn.Module):
    def __init__(self, nce_layers, num_patches=256, nc=256, init_type='normal', init_gain=0.02, gpu_ids=[]):
        ...
        self.init_gain = init_gain
        self.gpu_ids = gpu_ids
        self.neg_sample = nn.Parameter(torch.FloatTensor(len(nce_layers) + 2, 1, num_patches, nc), requires_grad=True)
